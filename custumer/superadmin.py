...
class RecenserListd(generics.RetrieveUpdateDestroyAPIView):
    ...
    filter_backends = [DjangoFilterBackend,SearchFilter]
    search_fields=['situation_matrimoniale','list_religion','religion','handicap','survie_de_mere','survie_de_pere','alphabetisation','niveau_instruction','status_occupation','occupation_actuelle','situation_occupation','branche_activite']
    def get_queryset(self):
        user = self.request.user
...
class CommoditeList(generics.ListAPIView):
    ...
    serializer_class=CommoditeS
    filter_backends = [DjangoFilterBackend,SearchFilter]
    search_fields=['nombre_piece_dormir','nature_mur','nature_toit','lieu_aisance','alimentation_eau','eclairage','cuisson']
    def get_queryset(self):
        user = self.request.user
        return Commodite.objects.all()
